...
layout(set = 0, binding = 0, std140) uniform Uniforms {
    ...
    FfxInt32x2     _OutputDims;        // 8 B  (OF output resolution)
    FfxInt32x2     _DepthDims;         // 8 B
    FfxFloat32x2   _InvDepthDims;      // 8 B
    FfxInt32x2     _ColorDims;         // 8 B
}ubo;

layout(set = 0, binding = 1)        uniform texture2D r_depth_tm1; // render size | R32_FLOAT
layout(location = 0)                out FFX_MIN16_F2 rw_motion_vectors; // OF size | RG16F
...
void ComputeMVHints(FfxInt32x2 output_pixel)
{
    ...
    rw_motion_vectors = motion;
}

// entry-point — viewport is set to OF output dims
void main()
{
    FfxInt32x2 output_pixel = FfxInt32x2(gl_FragCoord.xy);
