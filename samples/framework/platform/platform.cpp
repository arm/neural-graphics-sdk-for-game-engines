...
void Platform::set_app_argument(const std::string &key, const std::string &value)
{
	app_arguments[key] = value;
}

const std::unordered_map<std::string, std::string> &Platform::get_app_arguments() const
{
...
bool Platform::start_app()
{
	...
	options.benchmark_enabled = false;
	options.window            = window.get();
	options.app_arguments     = app_arguments;

	if (!active_app->prepare(options))
	{
